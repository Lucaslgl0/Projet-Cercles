...
for it in combi:
    cercle1_rayon = 1 / it[0]
    cercle2_rayon = 1 / it[1]
    cercle3_rayon = 1 / it[2]
    print(str(it[0]) + str(it[1]) + str(it[2]))
    ...
    plt.autoscale()
    # plt.show()
    print("Enregistrement de l'image... (" + str(it[0]) + str(it[1]) + str(it[2]) + ")")
    plt.savefig(
        "figure_chelous/fractale_infini"
        + str(it[0])
        + str(it[1])
        + str(it[2])
        + ".png",
